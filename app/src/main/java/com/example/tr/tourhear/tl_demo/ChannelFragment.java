...
import com.example.tr.tourhear.entity.ChannelExt;
import com.example.tr.tourhear.entity.ContactExt;
import com.example.tr.tourhear.entity.MsgCode;
import com.example.tr.tourhear.entity.ObservableHoriScrollView;
import com.example.tr.tourhear.entity.ScrollViewListener;
...
public class ChannelFragment extends Fragment implements OnChannelListener,
        OnSessionListener {
	...
	public void onResume() {
		...
		uiHandler = com.example.tr.tourhear.tl_demo.TLActivity.getUiHandler();
		if (channelApi == null) {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		if (container != null) { // && ((TLActivity)uiContext).isHorizonScreen()
			...
			// (TextView)mView.findViewById(R.id.presences);
			// memberButton.setOnClickListener(new
			// OnChannelMemberButtonPressed());

			// ObservableHoriScrollView channelsScroll =
			// (ObservableHoriScrollView)
			// mView.findViewById(R.id.channels_scroll);
	...
	private void updateChannelsHint(final int count) {
		((com.example.tr.tourhear.tl_demo.TLActivity)uiContext).runOnUiThread(new Runnable(){
			@Override
			public void run() {
				if (tvInitCh != null)
	...
	private class AsyncUpdateChannelState implements Runnable {
		...
		public void run() {
			...
			for (ChannelExt che1 : myChannels) {
				...
				tvNewCh.setEllipsize(TextUtils.TruncateAt.END);
	...
	private void onSessionPowerClicked(boolean wasOn, boolean act, int idx) {
		...
		if (!wasOn && !channelSelectIsBusy && selfOnline && currSession == null) {
			...
			((com.example.tr.tourhear.tl_demo.TLActivity) uiContext).stopDialogSession(selfId, 0);
		} else {
			if (!selfOnline)
	...
	private void uiEnterChannel(ChannelExt che1) {
		if (che1.cid.getType() != Constant.SESSION_TYPE_TOURLINK) {
			...
			((com.example.tr.tourhear.tl_demo.TLActivity) uiContext).showTourlinkChannelScroller(tlChannels
					.size());
		}
	}
	...
	public void onStartTalkFragment(int uid, int type, int sid) {
		if (myChannels == null) { // reconnect caused session-establish, and
			...
			Log.e(TAG,
					"myChannels is null, when onStartTalkFragment for session:"
							+ sid);
			return;
		}
		...
		alertDialogBuilder
				...
				.setPositiveButton(R.string.ch_create_seg, new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						channelApi.createPublicChannel(selfId,
								createChannel.name, newEC);
					}
				})
	...
	private void showChannelModifyDialog() {
		...
		if (mChs == null || mChs.size() < 1) {
			...
			((com.example.tr.tourhear.tl_demo.TLActivity) uiContext).showModifyChannelSpinner(mChs);
		}
	}

	private void showChannelDefaultDialog() {
		((com.example.tr.tourhear.tl_demo.TLActivity) uiContext).showSetDefaultSpinner();
	}

	private void showChannelDeleteDialog() {
		((com.example.tr.tourhear.tl_demo.TLActivity) uiContext)
	...
	private void syncCurrentSessionDelayed(int delayMs) {
		uiHandler.postDelayed(new Runnable() {
			...
			public void run() {
				if (sessionApi != null) {
					((com.example.tr.tourhear.tl_demo.TLActivity) uiContext).showProgressCircle();
	...
	public void onSessionGet(int selfUserId, int type, int sessionId, int initiator) {
		((com.example.tr.tourhear.tl_demo.TLActivity) uiContext).dismessProgressCircle();
	...
	private void funcGetAdverChannels() {
		if (selfOnline && myChannels == null) {
			if (!simpleChannelMode) {
				((com.example.tr.tourhear.tl_demo.TLActivity) uiContext).showProcessing(getResources().getString(R.string.getting_pub_channel));
				if (channelApi != null)
	...
	public void onAdverChannelsGet(int userId, Channel dfltChannel,
		...
		((com.example.tr.tourhear.tl_demo.TLActivity) uiContext).dismissProcessing();
	...
	private void funcGetChannelList() {
		if (selfOnline) {
			((com.example.tr.tourhear.tl_demo.TLActivity) uiContext).showProcessing(getResources().getString(R.string.getting_channels));
			if (channelApi != null)
	...
	private void mergeChannelList(Channel dfltCh, List<Channel> chs) {
		...
		List<Integer> myADCs = ((com.example.tr.tourhear.tl_demo.TLActivity)uiContext).getMyPreferedChannels();
	...
	@Override
	public void onPubChannelSearchResult(int uid,
			@SuppressWarnings("rawtypes") List channels) {
		@SuppressWarnings("unchecked")
		List<Channel> foundChs = (List<Channel>) channels;
		if (uid > 0 && foundChs.size() > 0) {
			List<Channel> diffChs = removeFocused(foundChs);
			if (diffChs.size() > 0)
				((com.example.tr.tourhear.tl_demo.TLActivity) uiContext).showFocusChannelSpinner(diffChs);
			else
